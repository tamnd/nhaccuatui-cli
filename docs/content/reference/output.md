...
describes what users get. Pick a format with `-o`, or let nhaccuatui choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nhaccuatui <command> -o table   # aligned columns for reading
nhaccuatui <command> -o jsonl   # one JSON object per line, for piping
nhaccuatui <command> -o json    # a single JSON array
nhaccuatui <command> -o csv     # spreadsheet friendly
nhaccuatui <command> -o tsv     # tab-separated
nhaccuatui <command> -o url     # just the URL column
nhaccuatui <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nhaccuatui <command> --fields id,title,url
```
...
```bash
nhaccuatui <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nhaccuatui <command>            # a table, because this is a terminal
nhaccuatui <command> | wc -l    # JSONL, because this is a pipe
```
